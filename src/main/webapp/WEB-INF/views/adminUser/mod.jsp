...
<form action="/adminUser/mod" method="post" class="form-horizontal">
	...
	<c:forEach items="${user.authList}" var="auth">
		...
		<c:if test="${!empty auth_withdraw}">
			<div class="alert alert-secondary" role="alert">
				<i class="fas fa-exclamation-circle"></i>탈퇴한 회원으로 회원 정보를 수정할 수 없습니다.
			</div>
		</c:if>
	<c:if test="${empty auth_withdraw}">	
	<div class="row formContainer">
	...
	<div class="row formContainer btnBox">
	    ...
	    <div class="col-sm-6 text-right">
	    	<button type="submit" class="btn btn-primary" data-oper="mod" ${!empty auth_withdraw ? 'disabled' : ''}>저장</button>
	    </div>
	</div>
